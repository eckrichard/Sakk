...
public class Kiralyno extends Babu{
    /**
     * Azok a lehetséges számok amelyekkel megkaphatjuk, hogy hova léphet a királynő.
     * Ebben az esetben ezeknek a számoknak a többszörösei is lehetnek.
     */
    private final static int[] LEHETSEGES_LEPES_KORDINATA_KIRALYNO = {-1, 1};

    public Kiralyno(final Csapat csapatBabu, final int x,final int y) {
        super(BabuTipus.KIRALYNO,x, y, csapatBabu, true);
    }

    public Kiralyno(final Csapat csapatBabu, final int x,final int y, final boolean elsoLepes) {
        super(BabuTipus.KIRALYNO,x, y, csapatBabu, elsoLepes);
    }
    ...
    public Collection<Lepes> szamoltErvenyesLepes(final Tabla tabla) {
        ...
        for (final int lehetosegek : LEHETSEGES_LEPES_KORDINATA_KIRALYNO){
    ...
    @Override
    public Kiralyno mozgoBabu(Lepes lepes) {
        return new Kiralyno(lepes.getMozgoBabu().getCsapatBabu(), lepes.getCelX(), lepes.getCelY(), false);
    }

    @Override
    public Kiralyno mozgoBabu(Lepes lepes, boolean elsoLepes) {
        return new Kiralyno(lepes.getMozgoBabu().getCsapatBabu(), lepes.getCelX(), lepes.getCelY(), elsoLepes);
    }

    public String toString(){ return BabuTipus.KIRALYNO.toString(); }
}
